...
namespace Unity.XR.PXR
{
    public class PXR_OverLay : MonoBehaviour, IComparable<PXR_OverLay>
    {
        ...
        private void OnDestroy()
        {
            ...
            PXR_Plugin.Render.UPxr_DestroyLayer(layerIndex);
        }
        ...
        private void InitializeBuffer()
        {
            switch (overlayShape)
            {
                ...
                case OverlayShape.Equirect:
                    for (int i = 0; i < layerTextureIds.Length; i++)
                    {
                        ...
                        {
                            PLog.w(TAG, string.Format("{0} Texture is null!", (EyeType)i));
                        }
                    }
                    break;
                default:
            ...
            PxrLayerParam layerParam = new PxrLayerParam();
            ...
            {
                layerParam.width = (uint)layerTextures[0].width;
                layerParam.height = (uint)layerTextures[0].height;
            }
            else
            {
            ...
            {
                layerParam.layerFlags = (UInt32)PxrLayerCreateFlags.PxrLayerFlagUseExternalImages;
                if ((layerTextures[0] != null && layerTextures[1] != null &&layerTextures[0] == layerTextures[1]) || layerTextures[1] == null)
                {
                    layerParam.layerLayout = LayerLayout.Mono;
                    layerParam.leftExternalImages = new UInt64[1]{(UInt64)layerTextures[0].GetNativeTexturePtr()};
                    layerParam.rightExternalImages = null;
                }
                else
                {
                    layerParam.layerLayout = LayerLayout.Stereo;
                    layerParam.leftExternalImages = new UInt64[1]{(UInt64)layerTextures[0].GetNativeTexturePtr()};
                    layerParam.rightExternalImages = new UInt64[1]{(UInt64)layerTextures[1].GetNativeTexturePtr()};
                }
            }
